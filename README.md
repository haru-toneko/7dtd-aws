...
Lambda: auto_stop          ← EventBridge 5分毎に起動
  │ SSM RunCommand
  ▼
EC2 (m7i-flex.large)       ← Docker + linuxserver/7-days-to-die
  │
...
max_players     = 6
game_world      = "Navezgane"       # "Navezgane" または "RWG" (ランダム生成)
...
- 初回起動はワールドデータ生成のため **10〜15 分**かかることがある
...
2. EC2 上でプレイヤーチェックスクリプトを手動実行:
   ...
   aws ssm send-command \
     ...
     --region ap-northeast-1 \
     --output json | jq .CommandId
   
   # 少し待ってから結果確認 (CommandIdを置き換え)
   aws ssm get-command-invocation \
     --command-id "上記のCommandId" \
...
```bash
# まず EC2 を停止
aws ec2 stop-instances --instance-ids i-0xxxxxxxxxxxxxxxxx --region ap-northeast-1
aws ec2 wait instance-stopped --instance-ids i-0xxxxxxxxxxxxxxxxx --region ap-northeast-1
...
├── scripts/
│   └── user_data.sh                 ← EC2 起動時スクリプト (Docker + 7DTD)
└── lambda/
